...
public class SelectionManager {
     ...
     * @return
     */
    public int getImageSelectPosition(String imagePath) {
        int index = -1;
        int size = CollectionUtil.size(mSelectImagePaths);
        for (int i = 0; i < size; i++) {
            String s = mSelectImagePaths.get(i);
            if (TextUtils.equals(s, imagePath)) {
                index=i;
                break;
            }
        }
