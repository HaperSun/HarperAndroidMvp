...
import static com.sun.media.camera.view.CameraView.TAKE_BOTH;
import static com.sun.media.camera.view.CameraView.TAKE_PHOTO;
import static com.sun.media.camera.view.CameraView.TAKE_VIDEO;
...
public class CaptureButton extends View {
    ...
    public CaptureButton(Context context, int size) {
        ...
        state = STATE_IDLE;
        //初始化按钮为可录制可拍照
        buttonState = TAKE_BOTH;
        LogHelper.i("CaptureButtom start");
        //默认最长录制时间为10s
        duration = 10 * 1000;
        LogHelper.i("CaptureButtom end");
        //默认最短录制时间为1.5s
        minDuration = 1500;
        centerX = (buttonSize + outsideAddSize * 2) / 2;
        centerY = (buttonSize + outsideAddSize * 2) / 2;
        rectF = new RectF(centerX - (buttonRadius + outsideAddSize - strokeWidth / 2),
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                state = STATE_PRESS;
                //判断按钮状态是否为可录制状态
                if ((buttonState == TAKE_VIDEO || buttonState == TAKE_BOTH)) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (captureListener != null && state == STATE_RECORDING
                        && (buttonState == TAKE_VIDEO || buttonState == TAKE_BOTH)) {
    ...
    private void handlerUnpressedByState() {
        ...
        switch (state) {
            //当前是点击按下
            case STATE_PRESS:
                if (captureListener != null && (buttonState == TAKE_PHOTO || buttonState ==
                        TAKE_BOTH)) {
