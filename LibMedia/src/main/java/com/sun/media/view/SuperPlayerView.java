...
import com.sun.media.controller.IControllerCallback;
import com.sun.media.controller.TCControllerFloat;
import com.sun.media.controller.TCControllerFullScreen;
import com.sun.media.controller.TCControllerWindow;
import com.sun.media.model.SuperPlayerConst;
import com.sun.media.model.SuperPlayerGlobalConfig;
import com.sun.media.model.SuperPlayerModel;
import com.sun.media.model.bean.TCResolutionName;
import com.sun.media.model.bean.TCVideoQuality;
import com.sun.media.net.TCLogReport;
import com.sun.media.protocol.IPlayInfoProtocol;
import com.sun.media.protocol.IPlayInfoRequestCallback;
import com.sun.media.protocol.TCPlayInfoParams;
import com.sun.media.protocol.TCPlayInfoProtocolV2;
import com.sun.media.protocol.TCPlayInfoProtocolV4;
import com.sun.media.utils.TCImageUtil;
import com.sun.media.utils.TCNetWatcher;
import com.sun.media.utils.TCUrlUtil;
import com.sun.media.utils.TCVideoQualityUtil;
...
public class SuperPlayerView extends RelativeLayout implements ITXVodPlayListener, ITXLivePlayListener {
     ...
     *
     * @param model
     * @param mStartPlayTime//起始播放时间
     */
    private int mStartPlayTime = 0;

    /**
     * @param model 视频数据
     * @param startPlayTime 开始播放时间，单位秒
     */
    public void playWithModel(final SuperPlayerModel model, int startPlayTime) {
...
//            updateVideoProgress(0, 0);//modify by tsliang
            updateVideoProgress(mStartPlayTime, 0);
...
//            mVodPlayer.setStartTime(0);//modify by tsliang
            mVodPlayer.setStartTime(mStartPlayTime);
    ...
    private void updateVideoProgress(long current, long duration) {
        ...
        if (mPlayerViewCallback != null) {
            mPlayerViewCallback.onPlayedTime((int) current, (int) duration);
        }
    }
    ...
    private void updatePlayState(int playState) {
        ...
        if (mPlayerViewCallback != null) {
            mPlayerViewCallback.onEndPlay(mCurrentPlayState);
        }
    }
    ...
    private IControllerCallback mControllerCallback = new IControllerCallback() {
        ...
        public void onSwitchPlayMode(int requestPlayMode) {
            if (mCurrentPlayMode == requestPlayMode) {
                return;
            }
            if (mLockScreen) {
                return;
    ...
    public void onPlayEvent(TXVodPlayer player, int event, Bundle param) {
        ...
        switch (event) {
            ...
            case TXLiveConstants.PLAY_EVT_PLAY_PROGRESS:
                ...
                updateVideoProgress(Math.round(progress / 1000f), Math.round(duration / 1000f));
                break;
    ...
    public void onPlayEvent(int event, Bundle param) {
        ...
        switch (event) {
            ...
            case TXLiveConstants.PLAY_EVT_PLAY_PROGRESS:
                ...
                updateVideoProgress(Math.round(progress / 1000f), Math.round(mMaxLiveProgressTime / 1000f));
                break;
            default:
                break;
        }
    ...
    public interface OnSuperPlayerViewCallback {
         ...
         * 播放结束
         */
        void onEndPlay(int currentState);

        /**
         * 已经播放时长
         */
        void onPlayedTime(int times, int totalTime);
    }
