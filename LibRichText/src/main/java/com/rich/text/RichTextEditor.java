...
public class RichTextEditor extends ScrollView {
    ...
    public RichTextEditor(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        rtTextHintColor = ta.getColor(R.styleable.RichTextEditor_rt_editor_text_hint_color, Color.parseColor("#757575"));
        ...
        actualWidth = getScreenWidth(mContext) - 50 * 2;
     ...
     * edittext初始化有内容
     */
    public void initEditText(String content) {
    ...
    private boolean isImagePathEmpty(String path) {
    ...
    public void insertImage(String imagePath, int width, int height) {
        ...
        try {
            ...
            if (lastEditStr.length() == 0) {
                //如果当前获取焦点的EditText为空，直接在EditText下方插入图片，并且插入空的EditText
                addEditTextAtIndex(lastEditIndex + 1, "");
                addImageViewAtIndex(lastEditIndex + 1, imagePath, width, height);
            } else if (editStr1.length() == 0) {
                //如果光标已经顶在了editText的最前面，则直接插入图片，并且EditText下移即可
                addImageViewAtIndex(lastEditIndex, imagePath, width, height);
                //同时插入一个空的EditText，防止插入多张图片无法写文字
                addEditTextAtIndex(lastEditIndex + 1, "");
            } else if (editStr2.length() == 0) {
                // 如果光标已经顶在了editText的最末端，则需要添加新的imageView和EditText
    ...
    public void addImageViewAtIndex(final int index, final String imagePath, final int width, final int height) {
        if (TextUtils.isEmpty(imagePath)) {
            return;
        }
        try {
            ...
            RelativeLayout root = imageLayout.findViewById(R.id.edit_root);
//			Log.e("====",  "图片：" + width + ", " + height);
            //计算缩放比例
            float sy = (float) (actualWidth * 0.1) / (float) (width * 0.1);
            //计算图片等比例放大后的高
            int imageHeight = (int) (height * sy);
//			RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
//					RelativeLayout.LayoutParams.MATCH_PARENT, imageHeight);//固定图片高度，记得设置裁剪剧中
//			imageView.setLayoutParams(params);
//			Log.e("====",  "图片2 ：" + actualWidth + ", " + imageHeight);
            XRichText.getInstance().loadImage(imagePath, imageView, rtImageHeight, root, true);
    ...
    public void addImageViewAtIndex(final int index, final String imagePath) {
        ...
        try {
            ...
            imageView.setAbsolutePath(imagePath);
            RelativeLayout root = imageLayout.findViewById(R.id.edit_root);
//			RelativeLayout editDefault = imageLayout.findViewById(R.id.edit_fail);
            //imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);//裁剪剧中
//			if (TextUtils.isEmpty(imagePath)) {
//				imageView.setVisibility(View.GONE);
//				editDefault.setVisibility(View.VISIBLE);
//			}else {
//				imageView.setVisibility(View.VISIBLE);
//				editDefault.setVisibility(View.GONE);
...
//				Log.d("LeiTest", "合并EditText");
     ...
     * 判断图片是否为空，为空则设置提示文字
     */
    private void notifyImageIsEmpty() {
        List<EditData> list = buildEditData();
        if (list.size() == 0) {
            defaultEditText.setHint(rtTextInitHint);
            return;
        }
        for (EditData unit : list) {
            if (!TextUtils.isEmpty(unit.imagePath)) {
                defaultEditText.setHint("");
                return;
            }
        }
        defaultEditText.setHint(rtTextInitHint);
