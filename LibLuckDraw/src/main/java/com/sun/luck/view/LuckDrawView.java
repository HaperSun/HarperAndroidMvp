...
public class LuckDrawView extends RelativeLayout {
    //当前的圆盘VIew
    private LuckDrawPanView mLuckDrawPanView;
    //开始按钮
    ...
    public void setRotateListener(RotateListener rotateListener) {
        mLuckDrawPanView.setRotateListener(rotateListener);
    ...
    private void init(Context context, AttributeSet attrs) {
        if ( attrs != null ) {
            ...
            try {
                mGoImgRes = typedArray.getResourceId(R.styleable.luckDrawView_goImg, 0);
            } finally { //回收这个对象
                typedArray.recycle();
            }
        }
        ...
        //添加圆盘视图
        mLuckDrawPanView = new LuckDrawPanView(context, attrs);
        LayoutParams layoutParams = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
        layoutParams.addRule(RelativeLayout.CENTER_IN_PARENT);
        mLuckDrawPanView.setLayoutParams(layoutParams);
        addView(mLuckDrawPanView);
    ...
    public void setConfig(Builder builder) {
        if ( builder.mColors != null ) {
            mLuckDrawPanView.setmColors(builder.mColors);
        }
        if ( builder.mDeses != null ) {
            mLuckDrawPanView.setmDeses(builder.mDeses);
        }
        if ( builder.mHuanImgRes != 0 ) {
            mLuckDrawPanView.setmHuanImgRes(builder.mHuanImgRes);
        }
        if ( builder.mIcons != null ) {
            mLuckDrawPanView.setmIcons(builder.mIcons);
        }
        if ( builder.mMainImgRes != 0 ) {
            mLuckDrawPanView.setmMainImgRes(builder.mMainImgRes);
        }
        if ( builder.mMinTimes != 0 ) {
            mLuckDrawPanView.setmMinTimes(builder.mMinTimes);
        }
        if ( builder.mTextColor != 0 ) {
            mLuckDrawPanView.setmTextColor(builder.mTextColor);
        }
        if ( builder.mTextSize != 0 ) {
            mLuckDrawPanView.setmTextSize(builder.mTextSize);
        }
        if ( builder.mType != 0 ) {
            mLuckDrawPanView.setmType(builder.mType);
        }
        if ( builder.mVarTime != 0 ) {
            mLuckDrawPanView.setmVarTime(builder.mVarTime);
        }
        if ( builder.mTypeNum != 0 ) {
            mLuckDrawPanView.setmTypeNum(builder.mTypeNum);
        }
        mLuckDrawPanView.show();
    }
    ...
    public void startRotate(int pisition) {
        if ( mLuckDrawPanView != null ) {
            mLuckDrawPanView.startRotate(pisition);
        }
    }
